...
_CONFIGS = [
    ...
    # 2b. SFT + MEM (skill-level with video memory)
    TrainConfig(
        ...
        save_interval=5000,
        val_log_interval=500,
        val_num_batches=5000,
        val_batch_size=2 * 32,
        val_episodes_index=list(range(0, 20)),
        freeze_filter=pi0_config.Pi0Config(pi05=True, action_horizon=32, video_memory_frames=1).get_freeze_filter(),
        ema_decay=None,
        checkpoint_base_dir="./outputs/checkpoints/pi05_b1k-all_skills_mem_K6",
        num_workers=8,
        batch_size=8*32,
    ),
    # 2c. SFT + MEM v2 (K=3, smaller stride, learnable temporal attention, frozen backbone)
    TrainConfig(
        name="pi05_b1k-all_skills_mem_K3_v2",
        exp_name="openpi_mem",
        project_name="B1K",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=32, video_memory_frames=3, video_memory_stride_s=0.2),
        data=LeRobotB1KDataConfig(
            repo_id="behavior-1k/2025-challenge-demos",
            base_config=DataConfig(
                prompt_from_task=True,
        ...
        ema_decay=None,
        checkpoint_base_dir="./outputs/checkpoints/pi05_b1k-all_skills_mem_K3_v2",
        num_workers=8,
        batch_size=8 * 32,
    ),
    # 3. RFT Configs
