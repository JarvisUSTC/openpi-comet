    ...
    def from_dict(cls, data: at.PyTree[ArrayT]) -> "Observation[ArrayT]":
        ...
        for key in data["image"]:
            ...
            if img.dtype == np.uint8:
                # Support multi-frame image tensors: [B, K, H, W, C] -> [B*K, H, W, C]
                if img.ndim == 5:
                    bsize, num_frames, height, width, channels = img.shape
                    img = img.reshape(bsize * num_frames, height, width, channels)
                data["image"][key] = img.astype(np.float32) / 255.0 * 2.0 - 1.0
            elif hasattr(img, "dtype") and img.dtype == torch.uint8:
                img = img.to(torch.float32)
                # Support multi-frame image tensors: [B, K, H, W, C] -> [B*K, C, H, W]
                if img.ndim == 5:
                    bsize, num_frames, height, width, channels = img.shape
                    img = img.reshape(bsize * num_frames, height, width, channels).permute(0, 3, 1, 2)
                else:
                    img = img.permute(0, 3, 1, 2)
                data["image"][key] = img / 255.0 * 2.0 - 1.0

        # Expand image masks to match reshaped multi-frame images: [B] -> [B*K]
        for key in data["image_mask"]:
            if key in data["image"]:
                img = data["image"][key]
                mask = data["image_mask"][key]
                if img.shape[0] != mask.shape[0]:
                    num_frames = img.shape[0] // mask.shape[0]
