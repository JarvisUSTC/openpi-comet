            ...
            try:
                ...
                for val_batch in val_loader:
                    with sharding.set_mesh(mesh):
                        val_metrics = peval_step(val_rng, train_state, val_batch)
                    val_losses.append(val_metrics)
                stacked = common_utils.stack_forest(val_losses)
                reduced_val = {k: float(v) for k, v in jax.device_get(jax.tree.map(jnp.mean, stacked)).items()}
                metrics_str = ", ".join(f"{k}={v:.4f}" for k, v in reduced_val.items() if not k.startswith("val/"))
