#!/usr/bin/env bash
# Train a LoRA on a group of similar skills (union filter).
#
# Usage:
#   scripts/train_skill_group_lora.sh "<group_name>" "<skill 1>" ["<skill 2>" ...] [-- extra train.py args...]
#
# Examples:
#   bash scripts/train_skill_group_lora.sh "open_close" "open door" "close door" "open drawer" "close drawer"
#   CONFIG_NAME=pi05_b1k-sampled_single_skill-lora bash scripts/train_skill_group_lora.sh "nav" "move to" "turn to" -- --num-train-steps 200
#
# Notes:
...
if [[ ${#SKILLS[@]} -eq 0 ]]; then
  echo "No skills provided. Usage: scripts/train_skill_group_lora.sh \"group\" \"skill1\" [\"skill2\" ...] [-- extra args]" >&2
  exit 2
fi

CONFIG_NAME="${CONFIG_NAME:-pi05_b1k-sampled_single_skill-lora}"
export CONFIG_NAME="${CONFIG_NAME}"
EXP_NAME="${EXP_NAME:-lora_${GROUP//[^a-zA-Z0-9_-]/_}_$(date +%Y%m%d_%H%M%S)}"
