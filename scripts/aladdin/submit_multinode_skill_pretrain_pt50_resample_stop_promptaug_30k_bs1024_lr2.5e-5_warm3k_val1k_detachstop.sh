...
#
# This variant reduces validation overhead (to smooth step-time jitter) and applies the stop-head
# stability tweaks discussed in the loss analysis:
# - detach stop head prefix from backbone
# - reduce stop loss weight
# - validate less frequently and with fewer/cheaper batches
#
# Target:
...
# - val_num_batches = 8
# - val_denoise_steps = 2
# - stop_detach_prefix = True
# - stop_loss_weight = 0.05
#
# Usage:
#   bash scripts/aladdin/submit_multinode_skill_pretrain_pt50_resample_stop_promptaug_30k_bs1024_lr2.5e-5_warm3k_val1k_detachstop.sh
#
# Optional env vars:
...
# Keep names short (Aladdin name max length is 64 chars) and keep the timestamp.
# If we have to truncate, preserve the suffix to avoid collisions.
EXP_NAME="${EXP_NAME:-b1k_pt50_resamp_detach_val1k_bs1024_lr2.5e-5_w3k_30k_$(date +%m%d_%H%M%S)}"
NAME="${NAME:-${EXP_NAME}}"
for var in NAME EXP_NAME; do
  v="${!var}"
  if (( ${#v} > 64 )); then
    # 64 = 49(prefix) + 1(_) + 14(suffix)
    v="${v:0:49}_${v: -14}"
  fi
  printf -v "${var}" "%s" "${v}"
done
...
EXTRA_ARGS_DEFAULT=(
  ...
  "--model.stop-detach-prefix"
  "--model.stop-loss-weight" "0.05"
