...
# - peak_lr = 2x baseline (2.5e-5 -> 5e-5)
# - num_train_steps = 30k
#
# Usage:
#   bash scripts/aladdin/submit_multinode_place_surface_stop_promptaug_30k_bs1024_lr5e-5.sh
#
# Optional env vars:
...
EXP_NAME="${EXP_NAME:-ps_stopaug_mn_bs1024_lr5e5_30k_$(date +%Y%m%d_%H%M%S)}"
NAME="${NAME:-${EXP_NAME}}"
if (( ${#NAME} > 64 )); then
  NAME="${NAME:0:64}"
fi
