#!/usr/bin/env bash
#
# Submit: place_surface + stop head + prompt aug
# Sampler: smart with base stride=4, overhead tuned to stay close to ~1/4 frames
# Steps: 60k -> 30k
#
# Usage:
#   bash scripts/aladdin/submit_place_surface_stop_promptaug_smart4_30k.sh
#
# Smart sampling knobs here are intentionally conservative:
# - base stride=4 with random phase
# - dense boundary margin=4 frames (ensures stop-head sees boundary context)
# - densify around 1 largest overall action-change and 1 gripper-change event with window=1

set -euo pipefail

export CONFIG_NAME="${CONFIG_NAME:-pi05_b1k-sampled_skill_group-stop-full-pretrained-prompt-aug}"
export EXP_NAME="${EXP_NAME:-place_surface_stop_promptaug_smart4_30k_$(date +%Y%m%d_%H%M%S)}"
export NAME="${NAME:-${EXP_NAME}}"

EXTRA_ARGS_DEFAULT=(
  "--num-train-steps" "30000"
  "--lr-schedule.decay-steps" "30000"
  "--data.frame-sampling" "smart"
  "--data.frame-stride" "4"
  "--data.frame-stride-jitter"
  "--data.frame-dense-boundary-margin-frames" "4"
  "--data.smart-num-action-events" "1"
  "--data.smart-action-event-window-frames" "1"
  "--data.smart-contact-num-events" "1"
  "--data.smart-contact-window-frames" "1"
)
if [[ -z "${EXTRA_ARGS:-}" ]]; then
  EXTRA_ARGS="$(printf "%q " "${EXTRA_ARGS_DEFAULT[@]}")"
fi
export EXTRA_ARGS

bash scripts/aladdin/submit_group_place_surface_stop.sh
