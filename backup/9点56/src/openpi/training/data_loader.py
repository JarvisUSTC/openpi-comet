...
def _supports_omnigibson_streaming() -> bool:
    try:
        import omnigibson.learning.utils.obs_utils  # noqa: F401
...
def create_behavior_dataset(data_config: _config.DataConfig, action_horizon: int) -> Dataset:
    ...
    use_keyframe_streaming = _supports_omnigibson_streaming()
    if not use_keyframe_streaming:
        logging.warning("omnigibson not found: fallback to non-streaming dataset mode.")
    # In the non-Omni fallback path we rely on LeRobot video decoding, and B1K
    # timestamps can drift by about one frame (~0.0333s). The default 1e-4
    # tolerance is too strict and causes otherwise valid samples to fail.
    video_tolerance_s = data_config.tolerance_s if use_keyframe_streaming else max(data_config.tolerance_s, 0.05)
    ...
    dataset = BehaviorLeRobotDataset(
        ...
        chunk_streaming_using_keyframe=use_keyframe_streaming,
        shuffle=True,
        ...
        if jax.process_count() > 1:
            ...
            if isinstance(dataset._dataset, MultiBehaviorLeRobotDataset):
                for dataset_index in range(len(dataset._dataset.datasets)):
                    inner_ds = dataset._dataset.datasets[dataset_index]._dataset
                    if hasattr(inner_ds, "chunks"):
                        indices = list(
                            range(
                                jax.process_index(),
                                len(inner_ds.chunks),
                        ...
                        inner_ds.chunks = [inner_ds.chunks[i] for i in indices]
                    else:
                        logging.info(f"[P{jax.process_index()}] Skipping chunk subset for dataset {dataset_index} (non-streaming mode).")
                if hasattr(dataset._dataset.datasets[0]._dataset, "chunks"):
                    total_chunks = sum(
                        len(dataset._dataset.datasets[di]._dataset.chunks)
                        for di in range(len(dataset._dataset.datasets))
                    )
                    logging.info(f"[P{jax.process_index()}] After subset, Dataset has {total_chunks} chunks.")
            else:
                inner_ds = dataset._dataset._dataset
                if hasattr(inner_ds, "chunks"):
                    indices = list(range(jax.process_index(), len(inner_ds.chunks), jax.process_count()))
                    inner_ds.chunks = [inner_ds.chunks[i] for i in indices]
                    logging.info(
                        f"[P{jax.process_index()}] After subset, Dataset has {len(inner_ds.chunks)} chunks."
                    )
                else:
                    logging.info(f"[P{jax.process_index()}] Skipping chunk subset (non-streaming mode).")
