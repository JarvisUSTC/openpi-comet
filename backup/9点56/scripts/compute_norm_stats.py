...
def create_torch_dataloader(
    ...
    if data_config.behavior_dataset_root is not None:
        dataset = _data_loader.create_behavior_dataset(data_config, action_horizon)
    elif hasattr(_data_loader, "create_torch_dataset"):
        dataset = _data_loader.create_torch_dataset(data_config, action_horizon, model_config)
    else:
        raise NotImplementedError(
            "This repository does not expose create_torch_dataset for non-B1K data. "
            "Please use a B1K config with behavior_dataset_root or add the missing dataset adapter."
        )
...
def create_rlds_dataloader(
    ...
    if not hasattr(_data_loader, "create_rlds_dataset"):
        raise NotImplementedError("This repository does not expose create_rlds_dataset.")
...
def main(config_name: str, max_frames: int | None = None):
    ...
    if data_config.behavior_dataset_root:
        try:
            ...
            print("[WARN] omnigibson not found, falling back to dataloader-based stats computation.")
